...
`GET /projects/{slug}/adapters/{id}/normalizers`, `POST` (body: `name`, `target` table, `columns[]`, `key_columns[]`, `mode`, `options`), `GET .../normalizers/{mid}`, `PATCH`, `DELETE`. Normalizer JSON per [19 §19.2](../technical-specs/19-import-pipeline.md); a column's `hash` transform carries `algorithm` (`bcrypt` | `argon2id` | `sha256` | `sha512` | `hmac_sha256`) and an optional `secret`. The normalizer saves without a `secret` even for `hmac_sha256`; the run fails then, at import time. There is no `salt` field — a normalizer's `sha256`/`sha512` hash cannot be salted; salting a hash is only `POST /tools/hash` ([17 §17.1](17-tools.md)).
