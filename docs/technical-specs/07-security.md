...
| Session | 256-bit random value, SHA-256 in `sessions`; cookie `testate_session`, `HttpOnly`, `SameSite=Strict`, `Secure` when the request arrived over HTTPS (trust proxy) and then named `__Host-testate_session` on a root-path deploy so no subdomain or other path can overwrite it, `Path` = base path; idle 12 h, absolute 7 d; touched at most once per minute |
