...
- The frontend is SolidJS 2.0 with Vite and Tailwind 4. Design tokens are the project's own, in a Tailwind `@theme` block; the design language is the project homepage's (ADR 0003). Components are hand-rolled Solid components. No component library, because every Solid component library still targets Solid 1.x. Each feature follows model, presenter, view: the model calls the API, the presenter owns signals and actions, the view is JSX only.
...
- A project holds one protected state named `init`. Adding a database adapter snapshots it into that state; changing an adapter's host, port, or database replaces its entry there. Both are allowed only while HEAD is `init` and the databases have not moved since, so every entry of `init` is one moment for every database; a project never restored is at its start by definition. HEAD moves to `init` only while the project has no HEAD. Renaming an adapter changes nothing else.
...
- Reused from sibling projects: the SolidJS 2.0 skill, the vendored anti-slop lint rules, the lefthook configuration, the Simulflow ssh2 patch, and the Reconflower key rotation procedure, which Testate documents in its own `KEY_ROTATION.md`.
