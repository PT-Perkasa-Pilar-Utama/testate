...
- The frontend is SolidJS 2.0 with Vite and Tailwind 4. Design tokens are the project's own, in a Tailwind `@theme` block; the design language is GitHub's (ADR 0002). Components are hand-rolled Solid components. No component library, because every Solid component library still targets Solid 1.x. Each feature follows model, presenter, view: the model calls the API, the presenter owns signals and actions, the view is JSX only.
