...
- A project holds one protected state named `init`. Adding a database adapter snapshots it into that state; changing an adapter's host, port, or database replaces its entry there. HEAD moves to `init` only while the project has no HEAD. Renaming an adapter changes nothing else.
