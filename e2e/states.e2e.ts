...
test.describe("state stories", () => {
  ...
  test("@story-80 @story-81 checkout history opens per-adapter results and the counters step with repair", async ({
    ...
    // The strategy is the preflight's to say; after the fact the table shows what took how long.
    await expect(detail.getByText(/\d+\.\d s/).first()).toBeVisible();
    ...
    // Nothing failed, so there is nothing to repair and no button offers to (story 81).
    await expect(counters.getByRole("button", { name: "Repair counters" })).toHaveCount(0);
