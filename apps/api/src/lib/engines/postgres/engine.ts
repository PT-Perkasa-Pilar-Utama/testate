...
import type { CheckoutRun, DbEngine, DisplayRow, RowText, SnapshotRun } from "../types.ts";
...
export function createPostgresEngine(netguard: Netguard): DbEngine {
  ...
  return {
    ...
    async introspect(conn, excluded) {
      ...
      return guarded("introspect", () => introspect(sql, excluded, conn.config.schemas));
    },
    ...
    async *readTable(conn, table, opts) {
      const sql = await pools.acquire(conn);
      const live = await introspect(sql, [], conn.config.schemas);
