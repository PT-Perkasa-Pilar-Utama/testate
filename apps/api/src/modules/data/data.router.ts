...
export function createDataRouter(h: DataHandlers): Hono {
  ...
  router.get(`${A}/saved-queries`, requireRole("viewer"), describe("data", "Saved queries", v.array(savedQuerySchema)), h.savedQueries);
  router.post(`${A}/saved-queries`, requireRole("qa"), describe("data", "Save a query", savedQuerySchema, 201), h.createSavedQuery);
  router.patch(`${A}/saved-queries/:qid`, requireRole("qa"), describe("data", "Update a saved query", savedQuerySchema), h.updateSavedQuery);
  router.delete(`${A}/saved-queries/:qid`, requireRole("qa"), describe("data", "Delete a saved query", v.undefined(), 204), h.removeSavedQuery);
  router.get(`${A}/query-history`, requireRole("viewer"), describe("data", "Query history", v.array(queryHistoryRowSchema)), h.history);
