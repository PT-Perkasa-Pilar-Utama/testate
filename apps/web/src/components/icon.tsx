...
/**
 * Icons, vendored from lucide (ISC). Generated from `lucide-static`'s `icon-nodes.json`; that
 * package is not a dependency and nothing of lucide ships at runtime except the markup below.
 *
 * `lucide-solid` exists and cannot be used here: it calls `mergeProps` and `splitProps`, which
 * Solid 2 does not export. Every Solid component library has that problem, which is why this
 * codebase hand-rolls its components (see the `design-system` skill).
 *
 * To add one: find it at https://lucide.dev, take its entry from `icon-nodes.json`, add a line.
 * Keep the list sorted and keep it short. An icon nobody uses is bytes for nothing.
 */
const ICONS = {
  activity:
...
export type IconProps = {
  name: IconName;
  /** Sized by the caller, so an icon can match the line it sits on. */
  class?: string | undefined;
  /** A label makes the icon the control's name. Without one it is decoration beside real text. */
  label?: string | undefined;
};

/**
 * 16px on lucide's 24px grid, stroked in the current text colour, so an icon takes the tone of
 * whatever contains it and needs no variant of its own.
 */
export default function Icon(props: IconProps): JSX.Element {
  return (
    <svg
      ...
      // A vendored constant from the generator above, never a value that came from a request.
