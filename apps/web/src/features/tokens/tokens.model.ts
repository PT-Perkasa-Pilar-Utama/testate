...
import type { ApiToken, JsonObject, TokenKind } from "@testate/shared";
...
import type { RevokedFilter, TokenSort } from "./tokens.presenter.ts";
...
export const tokensModel = {
  ...
  page: (
    ...
    params: TableParams<TokenSort>,
    kind: TokenKind | "",
    revoked: RevokedFilter
  ): Promise<Page<ApiToken>> =>
    apiClient.page("/tokens", apiTokenSchema, {
      ...tableQuery(params, cursor),
      kind: kind === "" ? undefined : kind,
      revoked: revoked === "" ? undefined : revoked,
    }),
